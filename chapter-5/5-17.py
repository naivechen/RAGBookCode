...
# 加载模型评估指标
evaluation_metrics = {'accuracy': 0.85, 'precision': 0.82, 'recall': 0.78}
# 可视化模型性能指标
st.subheader(' 模型性能指标 ')
st.write(pd.DataFrame.from_dict(evaluation_metrics, orient='index', columns=['Score']))
# 添加交互式输入组件
st.sidebar.subheader(' 模型参数调整 ')
param1 = st.sidebar.slider(' 参数 1', 0.0, 1.0, 0.5)
param2 = st.sidebar.slider(' 参数 2', 0.0, 1.0, 0.5)
# 使用用户输入的参数进行预测
prediction = model.predict([[param1, param2]])
# 显示预测结果
st.subheader(' 根据用户输入调整参数的预测结果 ')
st.write(' 参数 1:', param1)
st.write(' 参数 2:', param2)
st.write(' 预测结果 :', prediction[0])